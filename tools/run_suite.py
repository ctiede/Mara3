...
def run_script(template, subprog, exe='mara', runid='test', nodes=1, ppn=1, hours=8, **kwargs):
...
if __name__ == "__main__":
    ...
    for runid in suite['runs']:
        ...
        job_params          = suite['job_params']
        mara_opts['outdir'] = run_dir

        if suite['scaling_flag']: job_params.update(suite['runs'][runid])
        else: mara_opts.update(suite['runs'][runid])
